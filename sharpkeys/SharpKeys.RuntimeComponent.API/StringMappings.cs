...
namespace SharpKeys.RuntimeComponent.API
{
    public class StringMappings
    {
        ...
        public static void ReadRegistryBytes(IEnumerable<byte> bytes)
        {
            ...
            if (arrayOfBytes.Length > 8)
            {
                var nTotal = int.Parse(arrayOfBytes[8].ToString());
                for (int i = 0; i < nTotal - 1; i++)
                {
                    stringMappings.AddRegistryStringMapping(arrayOfBytes[(i * 4) + 12 + 0], arrayOfBytes[(i * 4) + 12 + 1], arrayOfBytes[(i * 4) + 12 + 2], arrayOfBytes[(i * 4) + 12 + 3]);
                }
            }
        }
        ...
        public StringKey GetStringKey(int scancode) => m_hashKeys.GetStringKey(scancode) ?? m_hashKeys.Get(scancode, string.Format("Unknown: {0}", string.Format("{0,4:X}", scancode).Replace(" ", "0")));
